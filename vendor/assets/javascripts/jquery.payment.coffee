...
formatBackCardNumber = (e) ->
  ...
  # Remove the digit + trailing spaaaaaace
  if /\d\s$/.test(value)
    e.preventDefault()
    setTimeout -> $target.val(value.replace(/\d\s$/, ''))
  # Remove digit if ends in spaaaaaace + digit
...
formatBackExpiry = (e) ->
  ...
  # Remove the trailing spaaaaaace + last digit
  if /\d\s\/\s$/.test(value)
    e.preventDefault()
...
restrictNumeric = (e) ->
  ...
  # Char is a number or a spaaaaaace
  !!/[\d\s]/.test(input)
